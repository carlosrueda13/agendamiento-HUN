...
async function assertConsultAppointments() {
  ...
  const response = sent.findLast((message) => /Dermatologia/.test(message.text || "")).text;
  ...
  assert(
    sent.at(-1).buttons?.some(
      (button) => button.id === lifecycle.ACTIONS.RETURN_TO_MENU
    ),
    "Consulta terminada debe permitir volver al menu."
  );
}

async function assertConsentOncePerSessionAndSecureClose() {
  const { deps, sent, sessions, consents } = createDeps();

  await handleIncomingMessage(buttonMessage("INTAKE_MENU_AGENDAR"), deps);
  await handleIncomingMessage(buttonMessage("INTAKE_CONSENT_ACCEPT"), deps);
  assert(consents.has("573001112233"), "Debe recordar consentimiento solo en memoria.");

  await handleIncomingMessage(
    buttonMessage(lifecycle.ACTIONS.RETURN_TO_MENU),
    deps
  );
  ...
  const result = await handleIncomingMessage(
    buttonMessage("INTAKE_MENU_CONSULTAR"),
    deps
  );
  const consentMessagesAfter = sent.filter(
    (message) => message.buttons?.[0]?.id === "INTAKE_CONSENT_ACCEPT"
  ).length;
  assert(result.step === "accepted_existing_consent", "Debe reutilizar consentimiento vigente.");
  assert(consentMessagesAfter === consentMessagesBefore, "No debe repetir consentimiento.");
  assert(sessions.get("573001112233").step === "awaiting_document_type");

  await handleIncomingMessage(buttonMessage(lifecycle.ACTIONS.CLOSE), deps);
  assert(!sessions.has("573001112233"), "Cerrar debe limpiar estado operativo.");
  assert(!consents.has("573001112233"), "Cerrar debe revocar consentimiento de sesion.");
  assert(/finalizada de forma segura/i.test(sent.at(-1).text));

  await handleIncomingMessage(buttonMessage("INTAKE_MENU_AGENDAR"), deps);
  assert(
    sent.at(-1).buttons?.[0]?.id === "INTAKE_CONSENT_ACCEPT",
    "Una conversacion nueva debe solicitar consentimiento otra vez."
  );
}
...
function assertHelpers() {
  ...
  const expiredConsents = new Map([
    ["573001112233", { session_id_hash: "hash", expires_at: 999 }],
  ]);
  assert(
    getActiveConsent(expiredConsents, "573001112233", 1000) === null &&
      !expiredConsents.has("573001112233"),
    "Consentimiento vencido debe eliminarse de memoria."
  );
}
...
async function main() {
  ...
  await assertConsentOncePerSessionAndSecureClose();
