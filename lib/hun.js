...
const BASE = process.env.HUN_API_BASE;
const API_KEY = process.env.HUN_API_KEY;
...
const ENDPOINTS = {
  ...
  recordatoriosFecha: "/webServiceFechaMedico/consultar",
...
function normalizeCups(row) {
  ...
  return cups.map((cup) => {
    ...
    const resolved = resolveProcedureDescription(normalizedCup);
    return {
      ...normalizedCup,
      codigo: resolved.code || getField(normalizedCup, ["codigo"]),
      descripcion: resolved.description,
      descripcion_fuente: resolved.source,
...
function createHunClient(httpClient = client) {
  return {
     ...
     * `descripcion` prioriza HUN y usa el catalogo oficial CUPS si HUN la omite.
     */
    ...
    /**
     * Consulta las citas de una ventana de fechas para recordatorios.
     * Los datos personales solo se devuelven al proceso llamador y no se loguean.
     */
    async consultarRecordatoriosVentana({ fechaInicial, fechaFinal } = {}) {
      if (!fechaInicial || !fechaFinal) {
        throw new HunApiError({
          message: "La consulta de recordatorios requiere fecha inicial y fecha final",
          method: "GET",
          endpoint: ENDPOINTS.recordatoriosFecha,
          category: "invalid_request",
        });
      }

      const rows = await requestRows(httpClient, ENDPOINTS.recordatoriosFecha, {
        params: {
          fecha_inicial: String(fechaInicial),
          fecha_final: String(fechaFinal),
        },
      });
      return normalizarLista(rows);
    },
...
function requireHunConfiguration() {
  const missing = ["HUN_API_BASE", "HUN_API_KEY"].filter(
    (key) => !String(process.env[key] || "").trim()
  );
  if (missing.length) {
    throw new HunApiError({
      message: `Configuracion HUN faltante: ${missing.join(", ")}`,
      method: "CONFIG",
      endpoint: "HUN",
...
function configured(method) {
  return async (...args) => {
    requireHunConfiguration();
    return hun[method](...args);
  };
}
...
module.exports = {
  ...
  normalizarLista,
  createHunClient,
  getEspecialidades: configured("getEspecialidades"),
  getAgendaPorEspecialidad: configured("getAgendaPorEspecialidad"),
  consultarCitasDocumento: configured("consultarCitasDocumento"),
  consultarCitaNumero: configured("consultarCitaNumero"),
  consultarRecordatoriosVentana: configured("consultarRecordatoriosVentana"),
  asignarCita: configured("asignarCita"),
  cancelarCita: configured("cancelarCita"),
  verificarCancelacion: configured("verificarCancelacion"),
  _private: {
    ENDPOINTS,
    getField,
    hunError,
