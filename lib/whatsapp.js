...
const DEFAULT_TIMEOUT_MS = Number(process.env.WHATSAPP_TIMEOUT_MS || 20000);

function buildReminderTemplatePayload({
  to,
  templateName,
  languageCode = "es_CO",
  fecha,
  ...
  const values = [fecha, hora, especialidad, nombre];
  if (values.some((value) => !String(value || "").trim())) {
    throw new Error(
      "El recordatorio requiere fecha, hora, especialidad y nombre."
    );
  }
