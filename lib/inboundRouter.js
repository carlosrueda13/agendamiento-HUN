...
const DOCUMENT_TYPE_BY_ID = Object.fromEntries(
  DOCUMENT_TYPE_OPTIONS.map((option) => [option.id, option])
);
const DOCUMENT_TYPE_BY_CODE = Object.fromEntries(
  DOCUMENT_TYPE_OPTIONS.map((option) => [option.type, option])
);
...
const runtimeConsents = new Map();

const CONSENT_TEXT =
  "🔒 *Autorización para continuar*\n\nAl seleccionar *Acepto*, autorizas al Hospital Universitario Nacional a tratar tus datos personales y datos sensibles de salud exclusivamente para consultar, gestionar, agendar, modificar o cancelar tus citas, de acuerdo con su política de tratamiento de datos personales.\n\nPuedes rechazar esta autorización y comunicarte directamente con la línea telefónica del hospital.";

function buildRejectText(phoneLine = DEFAULT_PHONE_LINE) {
  return `Entendemos tu decisión. 🙏\n\nSin la autorización no podemos continuar por WhatsApp. Puedes recibir atención directamente en la línea del hospital:\n\n📞 *${phoneLine}*`;
}
...
function getActiveConsent(consents, to, now = Date.now()) {
  const consent = consents.get(to);
...
function isReservedAppointment(item) {
  return normalizeState(item.estado) === "reservada";
}
...
function parseDocumentType(incoming = {}) {
  const byId = DOCUMENT_TYPE_BY_ID[incoming.interactiveId];
  if (byId) return byId;

  const text = normalizedText(incoming.text);
  return DOCUMENT_TYPE_OPTIONS.find(
    (option) =>
      normalizedText(option.type) === text || normalizedText(option.title) === text
  ) || null;
}
...
function parseDocumentNumber(text) {
  const document = String(text || "")
    ...
    .toUpperCase()
    .replace(/[\s.-]/g, "");
  return /^[A-Z0-9]{4,20}$/.test(document) ? document : null;
}
...
function formatAppointment(item, index) {
  const lines = [`*${index + 1}. ${item.especialidad || "Cita HUN"}*`];
  if (item.fecha) {
    lines.push(`📅 *Fecha:* ${item.fecha}${item.hora ? ` a las ${item.hora.slice(0, 5)}` : ""}`);
  }
  if (item.procedimiento) lines.push(`🩺 *Procedimiento:* ${item.procedimiento}`);
  if (item.medico) lines.push(`👩‍⚕️ *Profesional:* ${item.medico}`);
...
function formatCancelableAppointmentsMessage(citas) {
  if (!citas.length) {
    return "No encontramos citas disponibles para cancelar por WhatsApp. 🔎\n\nPuedes intentarlo nuevamente o comunicarte con la línea telefónica del hospital para recibir ayuda.";
  }

  return [
    "🗓️ *Estas son las citas que puedes cancelar:*",
    "",
    citas.map(formatAppointment).join("\n\n"),
    "",
    "Selecciona una opción para continuar. *No cancelaremos nada sin tu confirmación.*",
  ].join("\n");
}
...
function formatAppointmentsMessage(rows, now = new Date()) {
  const citas = upcomingAppointments(rows, now)
    .filter(isReservedAppointment)
    .slice(0, 5);
  if (!citas.length) {
    return "🔎 *No encontramos citas reservadas próximas.*\n\nSi consideras que debería aparecer alguna cita, intenta nuevamente o comunícate con la línea telefónica del hospital.";
  }

  return [
    "📋 *Tus próximas citas reservadas en HUN*",
    "",
    citas.map(formatAppointment).join("\n\n"),
    "",
    "Para modificar o cancelar una cita, vuelve al menú y selecciona la opción correspondiente. 😊",
  ].join("\n");
}

async function sendMenu(to, whatsapp, { continuation = false } = {}) {
  await whatsapp.sendInteractiveButtons({
    to,
    body: continuation
      ? "Continuemos con tu atención.\n\n¿Qué más deseas hacer?"
      : "👋 ¡Hola! Soy *Natalia*, asistente de citas del Hospital Universitario Nacional.\n\n¿Qué deseas hacer hoy?",
...
async function sendModifyCancelChoice(to, whatsapp) {
  await whatsapp.sendInteractiveButtons({
    to,
    body: "🗓️ *Gestionemos tu cita*\n\n¿Qué deseas hacer?",
    footer: "Hospital Universitario Nacional",
    buttons: MODIFY_CANCEL_BUTTONS,
  });
}

async function promptDocumentType(to, whatsapp, action) {
  const intro =
    action === ACTIONS.MODIFY_CANCEL
      ? "Para revisar tus citas antes de cancelar"
      : "Para consultar tus citas reservadas";
  await whatsapp.sendInteractiveList({
    to,
    body: `🪪 *Selecciona el tipo de documento*\n\n${intro}, elige el documento con el que estás registrado en HUN.`,
    footer: "Hospital Universitario Nacional",
...
async function promptDocumentNumber(to, whatsapp, documentType) {
  const option = DOCUMENT_TYPE_BY_CODE[documentType];
  await whatsapp.sendText(
    to,
    `Perfecto, seleccionaste *${option?.title || documentType}*. ✅\n\nAhora escribe únicamente el número del documento, sin puntos ni espacios.`
  );
}
...
async function sendCancelConfirmation(to, whatsapp, cita) {
  await whatsapp.sendInteractiveButtons({
    ...
    body: [
      ...
      "",
      formatAppointment(cita, 0),
      "",
      "La solicitud solo se enviará a HUN cuando confirmes.",
    ].join("\n"),
    footer: "Hospital Universitario Nacional",
    buttons: [
...
async function notifyLostCancellationContext(to, deps) {
  ...
  await deps.whatsapp.sendText(
    to,
    "⚠️ *Necesitamos verificar una cancelación anterior.*\n\nEl proceso se interrumpió. Inicia nuevamente la opción *Modificar/cancelar* o comunícate con la línea telefónica del hospital para confirmar el estado."
  );
  return true;
}
...
async function notifyLostRescheduleContext(to, deps) {
  ...
  await deps.whatsapp.sendText(
    to,
    "⚠️ *Tu modificación necesita revisión.*\n\nEl proceso se interrumpió y podrías tener una reserva pendiente. Comunícate con la línea telefónica del hospital antes de intentarlo nuevamente."
  );
  return true;
}
...
async function handleAcceptedConsent(to, session, deps) {
  ...
  deps.sessions.set(to, {
    action: session.action,
    step: "awaiting_document_type",
    session_id_hash: session.session_id_hash,
    expires_at: expiresAt(now(), ttlMs),
  });
  await promptDocumentType(to, whatsapp, session.action);
}
...
async function handleModifyCancelChoice(to, session, incoming, deps) {
  ...
  if (incoming.interactiveId === "INTAKE_CANCEL_APPOINTMENT") {
    ...
    await promptDocumentType(to, deps.whatsapp, session.action);
    return "cancel_document_type";
  }
...
async function handleDocumentTypeStep(to, session, incoming, deps) {
  const selected = parseDocumentType(incoming);
  if (!selected) {
    await promptDocumentType(to, deps.whatsapp, session.action);
    return;
  }
  ...
  deps.sessions.set(to, {
    ...session,
    step: "awaiting_document_number",
    document_type: selected.type,
    expires_at: expiresAt((deps.now || (() => Date.now()))(), deps.ttlMs),
  });
  await promptDocumentNumber(to, deps.whatsapp, selected.type);
}

async function handleDocumentNumberStep(to, session, incoming, deps) {
  const { whatsapp, hun, now = () => Date.now(), nowDate = () => new Date() } = deps;
  const document = parseDocumentNumber(incoming.text);

  if (!VALID_DOCUMENT_TYPES.has(session.document_type) || !document) {
    await promptDocumentNumber(to, whatsapp, session.document_type);
    return;
  }

  const parsed = { tipo: session.document_type, documento: document };
  ...
  try {
    ...
    if (session.action === ACTIONS.MODIFY_CANCEL) {
      ...
      if (!cancelables.length) {
        ...
        deps.sessions.delete(to);
        await sendCompletionActions(to, whatsapp);
        return;
      }
    ...
    await whatsapp.sendText(
      to,
      "⚠️ *No pudimos consultar tus citas en este momento.*\n\nIntenta nuevamente más tarde o comunícate con la línea telefónica del hospital."
    );
  } finally {
    if (session.action !== ACTIONS.MODIFY_CANCEL) {
      deps.sessions.delete(to);
...
async function handleCancelConfirmationStep(to, session, incoming, deps) {
  ...
  if (incoming.interactiveId === "CANCEL_CONFIRM_NO") {
    deps.sessions.delete(to);
    await whatsapp.sendText(to, "Entendido. 👍 *No se canceló ninguna cita.*");
    await sendCompletionActions(to, whatsapp);
    return;
  }
  ...
  if (!selected?.numeroCita || !isCancelableAppointment(selected)) {
    ...
    await whatsapp.sendText(
      to,
      "⚠️ No pudimos validar la cita seleccionada. Inicia nuevamente la opción *Modificar/cancelar*."
    );
    await sendCompletionActions(to, whatsapp);
    return;
  }
  ...
  if (!claimed) {
    ...
    await whatsapp.sendText(
      to,
      "⏳ *Tu solicitud de cancelación ya está en proceso.*\n\nTe avisaremos por este chat cuando HUN confirme el resultado."
    );
    return;
  }
  ...
  try {
    ...
    await whatsapp.sendText(
      to,
      "⏳ *Recibimos tu solicitud de cancelación.*\n\nHUN la está procesando y te avisaremos por este chat cuando tengamos el resultado."
    );
    ...
    await whatsapp.sendText(
      to,
      "⚠️ *No pudimos solicitar la cancelación en este momento.*\n\nIntenta nuevamente más tarde o comunícate con la línea telefónica del hospital."
    );
    await sendCompletionActions(to, whatsapp);
  }
}
...
async function handleIncomingMessage(message, deps = {}) {
  ...
  const consents = deps.consents || runtimeConsents;
  deps.sessions = sessions;
  deps.consents = consents;
  ...
  if (
    ...
    throw new Error(
      "whatsapp debe exponer sendText, sendInteractiveButtons y sendInteractiveList."
    );
  }
  ...
  const lifecycleAction = parseLifecycleAction(incoming);

  if (lifecycleAction === LIFECYCLE_ACTIONS.RETURN_TO_MENU) {
    sessions.delete(to);
    await sendMenu(to, whatsapp, { continuation: true });
    return { handled: true, step: "menu_returned" };
  }

  if (lifecycleAction === LIFECYCLE_ACTIONS.CLOSE) {
    sessions.delete(to);
    consents.delete(to);
    await whatsapp.sendText(to, buildClosedMessage());
    return { handled: true, step: "conversation_closed" };
  }
  ...
  if (incoming.action) {
    ...
    const activeConsent = getActiveConsent(consents, to, now());
    if (activeConsent) {
      actionSession.session_id_hash = activeConsent.session_id_hash;
      sessions.set(to, actionSession);
      await handleAcceptedConsent(to, actionSession, deps);
      return {
        handled: true,
        action: incoming.action,
        step: "accepted_existing_consent",
      };
    }
  ...
  if (session.step === "awaiting_document_type") {
    await handleDocumentTypeStep(to, session, incoming, deps);
    return { handled: true, action: session.action, step: "document_type" };
  }

  if (session.step === "awaiting_document_number") {
    await handleDocumentNumberStep(to, session, incoming, deps);
    return { handled: true, action: session.action, step: "document_number" };
  }
