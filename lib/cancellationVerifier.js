...
async function finishOperation(operation, state, details, deps) {
  ...
  const message = state === "cancelada"
    ? "✅ *Tu cita fue cancelada correctamente en HUN.*\n\nYa no necesitas realizar ninguna acción adicional."
    : "⚠️ *No pudimos confirmar la cancelación en HUN.*\n\nInicia el proceso nuevamente o comunícate con la línea telefónica del hospital para verificar el estado de tu cita.";
  ...
  try {
    await sendCompletionActions(operation.to, deps.whatsapp);
  } catch (error) {
    console.error("No se pudieron enviar acciones de cierre de cancelacion:", error.message);
  }
