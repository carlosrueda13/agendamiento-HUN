...
const { resolveProcedureDescription } = require("./cupsCatalog");
const db = require("./db");
...
const FLOW_ERROR_MESSAGES = {
  ...
  no_procedures:
    "No hay procedimientos disponibles para esta especialidad. Elige otra especialidad o intenta mas tarde.",
  procedure_names_missing:
    "HUN no informo los nombres de los procedimientos disponibles. Elige otra especialidad o intenta mas tarde.",
  eps_missing:
...
function signSelectionIdentity(prefix, identity, flowToken, expiresAt) {
  const exp = Math.floor(new Date(expiresAt).getTime() / 1000);
  const payload = JSON.stringify(canonicalJson({
    exp,
    flow_token_ref: sessionRef(flowToken),
  ...
  return `${prefix}_${exp.toString(36)}_${signature}`;
}
...
function normalizeProcedureCode(value) {
  return String(hun.limpiar(value) || "").toUpperCase();
}

function dateVisibleTitle(value) {
...
function buildSlotsFromAgenda({
  ...
  for (const row of agenda) {
    for (const cup of row.cups || []) {
      ...
      if (
        procedureCode &&
        normalizeProcedureCode(cup.codigo) !== normalizeProcedureCode(procedureCode)
      ) {
        continue;
      }
      ...
      const descripcion = resolveProcedureDescription(cup).description;
  ...
  const limitedRows = maxSlots === null ? filas : filas.slice(0, Math.max(0, maxSlots));
...
function buildProceduresFromAgenda({
  agenda,
  codEsp,
  flowToken,
  expiresAt,
  ...
  for (const row of Array.isArray(agenda) ? agenda : []) {
    for (const cup of row.cups || []) {
      ...
      if (!esAutogestionable && !permitirNoAutogestionable) continue;
      const identity = slotIdentityFromAgenda(row, cup, codEsp);
      if (!identity || !slotEsFuturo(identity)) continue;

      const code = normalizeProcedureCode(identity.procedimiento);
      if (!code) continue;
      const resolved = resolveProcedureDescription(cup);
      const name = hun.limpiar(resolved.description);
      if (!name) {
        unresolvedCount += 1;
        continue;
      }
      const specialtyName = hun.limpiar(row.nombre_especialidad);
      ...
      if (!existing) {
        ...
        existing.count += 1;
        if (resolved.source === "hun" && existing.nameSource !== "hun") {
          existing.name = name;
          existing.nameSource = "hun";
        } else if (
          resolved.source === existing.nameSource &&
          name.localeCompare(existing.name) < 0
        ) {
          existing.name = name;
        }
        if (!existing.specialtyName && specialtyName) existing.specialtyName = specialtyName;
      }
    }
  }

  const candidates = {};
  const procedures = [...grouped.values()]
    .sort((a, b) => a.name.localeCompare(b.name) || a.code.localeCompare(b.code))
    .map((procedure) => {
      const id = signSelectionIdentity(
        "procedure_v1",
        { especialidad: String(codEsp), procedimiento: procedure.code },
        flowToken,
        expiresAt
      );
      candidates[id] = procedure;
      return { id, title: procedure.name.slice(0, 80) };
    });

  return { procedures, candidates, unresolvedCount };
}
...
function buildProcedureAvailability({
  ...
  const all = buildSlotsFromAgenda({
    agenda,
    codEsp,
    flowToken,
    expiresAt,
  ...
  for (const slot of all.slots) {
    const candidate = all.candidatos[slot.id];
    if (!candidate) continue;
    if (!slotsByDate.has(candidate.fecha)) slotsByDate.set(candidate.fecha, []);
    slotsByDate.get(candidate.fecha).push(slot);
  }
  ...
  const dates = [...slotsByDate.keys()].sort().map((date) => {
    ...
    const id = signSelectionIdentity(
      ...
      {
        especialidad: String(codEsp),
        procedimiento: normalizeProcedureCode(procedureCode),
        fecha: date,
      },
      flowToken,
      expiresAt
    );
    dateCandidates[id] = date;
    return {
      id,
      title: dateVisibleTitle(date).slice(0, 80),
  ...
  return {
    ...
    dateCandidates,
    slotsByDate,
    candidates: all.candidatos,
  };
}

function slotsForDate(availability, date) {
  const visible = (availability.slotsByDate.get(date) || []).map((slot) => {
    const candidate = availability.candidates[slot.id];
    const doctor = candidate?.medico || "Profesional HUN";
    return {
      id: slot.id,
      title: `${horaCorta(candidate?.hora)} - ${doctor}`.slice(0, 80),
      description: candidate?.consultorio
        ? `Consultorio ${candidate.consultorio}`
        : "Consulta HUN",
    };
  });
...
async function buildSlotsVigentes(flowToken, session) {
  ...
  let result;
  if (
    session.flow_tipo !== "campania" &&
    session.procedimiento_codigo &&
    session.fecha_seleccionada
  ) {
    const availability = buildProcedureAvailability({
      agenda,
      codEsp: session.especialidad_codigo,
      procedureCode: session.procedimiento_codigo,
      flowToken,
      expiresAt: session.expires_at,
      permitirNoAutogestionable,
    });
    result = slotsForDate(availability, session.fecha_seleccionada);
  } else {
    result = buildSlotsFromAgenda({
      agenda,
      codEsp: session.especialidad_codigo,
      flowToken,
      expiresAt: session.expires_at,
...
async function slotNoDisponibleResponse(flowToken, session, version, slots = null) {
  ...
  if (slotsVigentes.length) {
    if (
      ...
      return {
        ...
        screen: "SLOTS",
        data: {
          ...autoSlotScreenData(session, slotsVigentes),
  ...
  if (session?.procedimiento_codigo) {
    const agenda = await hun.getAgendaPorEspecialidad(
      session.especialidad_codigo,
      fechaFinalConsulta()
    );
    ...
    saveRuntimeSession(flowToken, {
      date_candidates: availability.dateCandidates,
      fecha_token: null,
    ...
    return {
      ...
      screen: "FECHA",
      data: {
        ...dateScreenData(session, availability.dates),
        error_message: FLOW_ERROR_MESSAGES.slot_unavailable,
      },
    };
  }
...
async function hunAvailabilityErrorResponse(flowToken, version, error, endpointLogico, session = null) {
  ...
  if (session?.procedimiento_nombre && session?.fecha_seleccionada) {
    return {
      version,
      screen: "SLOTS",
      data: {
        ...autoSlotScreenData(session, []),
        error_message: FLOW_ERROR_MESSAGES.hun_availability,
      },
    };
  }

  if (session?.procedimiento_nombre) {
    return {
      version,
      screen: "FECHA",
      data: {
        ...dateScreenData(session, []),
        error_message: FLOW_ERROR_MESSAGES.hun_availability,
      },
    };
  }
...
async function cancelarCitaE2eSiAplica(flowToken, numeroCita, session, started) {
  ...
  try {
    await hun.cancelarCita(
      numeroCita,
      session.paciente?.tipo_documento,
      session.paciente?.numero_documento
    );
    await guardarEventoFlow(flowToken, {
      event_type: "cancelacion_e2e",
      status: "exitosa",
...
async function actualizarEstadoDestinatarioCampania(recipientId, estado) {
  if (!recipientId) return false;

  try {
    const updated = await db.actualizarEstadoDestinatario(recipientId, estado);
    if (!updated?.id) {
      console.error(
        `No se pudo actualizar estado de destinatario de campania estado=${estado}`
      );
      return false;
    }
    return true;
...
async function handleFlow(payload) {
  ...
  if (rescheduleHandler.isRescheduleScreen(screen)) {
    return rescheduleHandler.handleFlow(payload);
  }
  ...
  if (action === "data_exchange") {
    ...
    try {
      switch (screen) {
        ...
        case "PROCEDIMIENTO":
          return await pasoProcedimiento(flowToken, data, version);
        case "FECHA":
          return await pasoFecha(flowToken, data, version);
        case "SLOTS":
...
async function pasoIdentificacionCampania(flowToken, data, version, campaignContext) {
  ...
  await actualizarEstadoDestinatarioCampania(
    campaignContext.recipient_id,
    "flow_iniciado"
  );
...
async function sendTerminalTextConEvento(flowToken, message, evento = {}) {
  const destination = getRuntimeSession(flowToken)?.whatsapp_numero || flowToken;
  const sent = await sendTextConEvento(flowToken, message, evento);

  try {
    const actionsSent = await sendCompletionActions(destination, wa);
    await guardarEventoFlow(flowToken, {
      event_type: "whatsapp_acciones_cierre",
      status: actionsSent ? "enviado" : "fallida",
      source: "whatsapp",
...
function procedureScreenData(specialtyName, procedures) {
  return {
    especialidad_seleccionada: specialtyName
...
function dateScreenData(session, dates) {
  return {
    procedimiento_seleccionado: `Procedimiento: ${session.procedimiento_nombre}`,
    fechas: dates,
  };
}

function autoSlotScreenData(session, slots) {
  return {
    procedimiento_seleccionado: `Procedimiento: ${session.procedimiento_nombre}`,
    fecha_seleccionada: `Fecha: ${dateVisibleTitle(session.fecha_seleccionada)}`,
    slots,
  };
}

async function listarProcedimientosEspecialidad(flowToken, session, codEsp, version) {
  let agenda = [];
  try {
    ...
    return hunAvailabilityErrorResponse(
      ...
      version,
      error,
      "get_agenda_procedimientos",
      session
    );
  }

  const permitirNoAutogestionable = permiteWaiverNoAutogestionable(session);
  const result = buildProceduresFromAgenda({
    agenda,
    codEsp,
    flowToken,
    expiresAt: session.expires_at,
  ...
  const specialtyName = Object.values(result.candidates).find(
    (candidate) => candidate.specialtyName
  )?.specialtyName || null;

  saveRuntimeSession(flowToken, {
    especialidad_codigo: codEsp,
    especialidad_nombre: specialtyName,
    procedure_candidates: result.candidates,
    procedimiento_token: null,
    procedimiento_codigo: null,
  ...
  await guardarEventoFlow(flowToken, {
    ...
    status: result.procedures.length ? "exitosa" : "fallida",
    endpoint_logico: "flow_especialidad",
    especialidad_codigo: codEsp,
    estado_contacto: "flow_especialidad",
    resultado_operativo: result.procedures.length
      ? "procedimientos_disponibles"
      : "sin_procedimientos",
    motivo_fallo_simple: result.procedures.length
      ? null
      : result.unresolvedCount
        ? "nombres_procedimiento_no_disponibles"
        : "sin_procedimientos",
    conteo_resultados: {
      procedimientos: result.procedures.length,
      procedimientos_omitidos_sin_nombre: result.unresolvedCount,
    },
  });
  ...
  if (!result.procedures.length) {
    return {
      ...
      data: {
        especialidades: await recuperarEspecialidadesParaRetry(),
        error_message: result.unresolvedCount
          ? FLOW_ERROR_MESSAGES.procedure_names_missing
          : FLOW_ERROR_MESSAGES.no_procedures,
      },
    };
  }
  ...
  return {
    ...
    data: procedureScreenData(specialtyName, result.procedures),
  };
}
...
async function pasoEspecialidad(flowToken, data, version) {
  ...
  return listarProcedimientosEspecialidad(flowToken, session, codEsp, version);
}

async function pasoProcedimiento(flowToken, data, version) {
  const session = getRuntimeSession(flowToken);
  const procedureToken = String(data.procedimiento_token || "");
  if (!session?.paciente || !session.especialidad_codigo || !procedureToken) {
    return missingRuntimeResponse(version);
  }

  let agenda = [];
  try {
    agenda = await hun.getAgendaPorEspecialidad(
      session.especialidad_codigo,
      fechaFinalConsulta()
    );
  } catch (error) {
    return hunAvailabilityErrorResponse(
      ...
      version,
      error,
      "reconsultar_agenda_procedimiento",
      session
    );
  }

  const permitirNoAutogestionable = permiteWaiverNoAutogestionable(session);
  const procedures = buildProceduresFromAgenda({
    agenda,
    codEsp: session.especialidad_codigo,
    flowToken,
    expiresAt: session.expires_at,
  ...
  const selected = procedures.candidates[procedureToken];

  if (!selected) {
    saveRuntimeSession(flowToken, { procedure_candidates: procedures.candidates });
    if (!procedures.procedures.length) {
      return {
        ...
        data: {
          especialidades: await recuperarEspecialidadesParaRetry(),
          error_message: FLOW_ERROR_MESSAGES.no_procedures,
        },
      };
    }
    return {
      version,
      screen: "PROCEDIMIENTO",
      data: procedureScreenData(session.especialidad_nombre, procedures.procedures),
    };
  }
  ...
  const freshSession = saveRuntimeSession(flowToken, {
    ...
    procedimiento_token: procedureToken,
    procedimiento_codigo: selected.code,
    procedimiento_nombre: selected.name,
    date_candidates: availability.dateCandidates,
    fecha_token: null,
    fecha_seleccionada: null,
    candidatos: {},
  ...
  await guardarEventoFlow(flowToken, {
    event_type: "flow_procedimiento",
    status: availability.dates.length ? "exitosa" : "fallida",
    endpoint_logico: "flow_procedimiento",
    especialidad_codigo: session.especialidad_codigo,
    resultado_operativo: availability.dates.length
      ? "fechas_disponibles"
      : "sin_fechas",
    motivo_fallo_simple: availability.dates.length ? null : "sin_fechas",
    conteo_resultados: { fechas: availability.dates.length },
  });
  ...
  if (!availability.dates.length) {
    return {
      version,
      screen: "PROCEDIMIENTO",
      data: procedureScreenData(
        freshSession.especialidad_nombre,
  ...
  return {
    version,
    screen: "FECHA",
    data: dateScreenData(freshSession, availability.dates),
  };
}

async function pasoFecha(flowToken, data, version) {
  const session = getRuntimeSession(flowToken);
  const dateToken = String(data.fecha_token || "");
  if (
    !session?.paciente ||
    !session.especialidad_codigo ||
    !session.procedimiento_codigo ||
  ...
  const selectedDate = availability.dateCandidates[dateToken];
  ...
  if (!selectedDate) {
    ...
    if (!availability.dates.length) {
      return {
        ...
        screen: "PROCEDIMIENTO",
        data: procedureScreenData(
          session.especialidad_nombre,
          buildProceduresFromAgenda({
            agenda,
            codEsp: session.especialidad_codigo,
            flowToken,
            expiresAt: session.expires_at,
    ...
    return {
      version,
      screen: "FECHA",
      data: dateScreenData(session, availability.dates),
    };
  }
  ...
  await guardarEventoFlow(flowToken, {
    event_type: "flow_fecha",
    status: "exitosa",
    endpoint_logico: "flow_fecha",
    especialidad_codigo: session.especialidad_codigo,
    resultado_operativo: "horarios_fecha_disponibles",
    conteo_resultados: { horarios: result.slots.length },
  });

  return {
    version,
    screen: "SLOTS",
    data: autoSlotScreenData(freshSession, result.slots),
  };
}
...
async function pasoConfirmar(flowToken, version) {
  ...
  procesarCita(flowToken).catch(async (error) => {
    ...
    await sendTerminalTextConEvento(flowToken, FLOW_ERROR_MESSAGES.assignment_failed, {
      especialidad_codigo: freshSession.especialidad_codigo,
      resultado_operativo: "error_asignacion",
      motivo_fallo_simple: "error_inesperado",
    });
...
async function procesarCita(flowToken) {
  ...
  if (!elegido || !paciente) {
    ...
    await sendTerminalTextConEvento(
      flowToken,
...
async function asignarYConfirmar(flowToken, payload, elegido, session, started) {
  try {
    ...
    let correo = session.contacto_email || null;
    if (!correo) {
      try {
        correo = await db.getContactoEmailSesion(flowToken);
      } catch (error) {
        console.warn(
          `Correo de confirmacion no recuperado session=${sessionRef(
            flowToken
          )} reason=contacto_temporal_no_disponible`
        );
      }
    }
    ...
    const confirmacionWhatsappEnviada = await sendTerminalTextConEvento(
      flowToken,
      `✅ *¡Tu cita quedó agendada!*\n\n` +
        `🏥 *Especialidad:* ${detalleCita.especialidad}\n` +
        `👩‍⚕️ *Profesional:* ${detalleCita.medico}\n` +
        `🩺 *Tipo de procedimiento:* ${detalleCita.procedimiento}\n` +
        `📅 *Fecha:* ${detalleCita.fecha} a las ${horaCorta(detalleCita.hora)}\n` +
    ...
    const emailResult = await enviarConfirmacion({
      to_email: correo,
    ...
    console.log(
      `Resultado correo confirmacion session=${sessionRef(flowToken)} status=${
        emailResult?.sent ? "enviado" : emailResult?.reason || "fallido"
      }`
    );
