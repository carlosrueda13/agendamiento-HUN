...
function buildProceduresFromAgenda({
  ...
  for (const row of Array.isArray(agenda) ? agenda : []) {
    for (const cup of row.cups || []) {
      ...
      if (!code) continue;
      const name = hun.limpiar(cup.descripcion) || "Procedimiento disponible";
      const specialtyName = hun.limpiar(row.nombre_especialidad);
      const existing = grouped.get(code);
      if (!existing) {
        grouped.set(code, { code, name, specialtyName, count: 1 });
      } else {
        existing.count += 1;
        if (name.localeCompare(existing.name) < 0) existing.name = name;
...
async function listarProcedimientosEspecialidad(flowToken, session, codEsp, version) {
  ...
  await guardarEventoFlow(flowToken, {
    ...
    resultado_operativo: result.procedures.length
      ...
      : "sin_procedimientos",
    motivo_fallo_simple: result.procedures.length ? null : "sin_procedimientos",
    conteo_resultados: { procedimientos: result.procedures.length },
  });
